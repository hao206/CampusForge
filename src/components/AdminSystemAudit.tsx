...
import { Mentor, NotificationAudience, Resource, Role, ReviewStatus } from '../types';
...
export const AdminSystemAuditModule: React.FC<AdminSystemAuditProps> = ({ t, accentColor }) => {
  ...
  const [mentors, setMentors] = useState<Mentor[]>(
    INITIAL_MENTORS.map((mentor, index) => ({
      ...mentor,
      reviewStatus: index === 2 ? 'Pending' : 'Approved',
    }))
  );
  ...
  const stats = [
    ...
    { label: 'Total Mentors', value: mentors.length, detail: `${mentors.filter((m) => m.reviewStatus === 'Approved').length} approved`, icon: ShieldCheck },
  ...
  const handleMentorReview = (mentorId: string, reviewStatus: ReviewStatus) => {
    setMentors((prev) => prev.map((mentor) => (mentor.id === mentorId ? { ...mentor, reviewStatus } : mentor)));
    logAdminAction(`${reviewStatus} mentor ${mentorId}`, 'Mentor Management');
  };
  ...
  return (
    <div className="p-1 md:p-4 space-y-6 font-sans">
      ...
      {activeTab === 'mentors' && (
        <ListPanel title="Mentor Management">
          {mentors.map((mentor) => (
            <div key={mentor.id} className="rounded-xl border border-white/5 bg-[#0A0A0A] p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <img src={mentor.avatar} alt={mentor.name} className="h-10 w-10 rounded-full object-cover" />
                <div>
                  <h3 className="text-sm font-bold text-white">{mentor.name}</h3>
                  <p className="text-[10px] text-slate-500">{mentor.sessionsBooked} sessions - rating {mentor.rating}</p>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <span className={`rounded-lg border px-2 py-1 text-[9px] font-bold ${statusClass(mentor.reviewStatus)}`}>{mentor.reviewStatus}</span>
                <AdminButton tone="success" onClick={() => handleMentorReview(mentor.id, 'Approved')}>Approve</AdminButton>
                <AdminButton tone="danger" onClick={() => handleMentorReview(mentor.id, 'Rejected')}>Reject</AdminButton>
              </div>
            </div>
          ))}
        </ListPanel>
