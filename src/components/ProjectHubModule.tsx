...
export const ProjectHubModule: React.FC<ProjectHubProps> = ({
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    ...
    const parsedSkills = projSkills.split(',').map(s => s.trim()).filter(Boolean);
    createProject({
      name: projName,
      ...
      status: 'Recruiting',
    }, user?.id || 'u1', user?.fullName || 'Alex Nguyen');

    addLog(`Created new study project proposal: ${projName}`, 'Project Hub', user?.fullName || 'Academic Peer');
    addReputation(150);
    addToast(lang === 'en' ? `Project created successfully +150 XP!` : `Đăng ký đề tài thành công +150 XP!`, 'success');

    setShowCreateModal(false);
    // Reset fields
    setProjName('');
    setProjDesc('');
    setProjSkills('React, NodeJS');
  };
  ...
  const confirmApplication = (projId: string) => {
    setApplyStates(prev => ({ ...prev, [projId]: 'applying' }));
    
    setTimeout(() => {
      applyProject(projId, applyRemark, user?.fullName || 'Academic Peer');
      setApplyStates(prev => ({ ...prev, [projId]: 'done' }));
      setPendingApplyProjId(null);
      
      const matchedProj = projects.find(p => p.id === projId);

      addLog(`Submitted team entry application for project ID: ${projId}`, 'Project Hub', user?.fullName || 'Academic Peer');
      addReputation(50);
      addToast(lang === 'en' ? 'Submitted team joining application +50 XP!' : 'Đơn gia nhập nhóm đã được gửi +50 XP!', 'success');

      // Programmatic trigger (Part B4): Notify Project Leader about application
      if (matchedProj) {
        // Log notification
        addNotification(
          lang === 'en' ? 'New Project Application' : 'Đơn ứng tuyển đề tài mới',
          `${user?.fullName || 'Academic Peer'} applied to join academic workspace "${matchedProj.name}". Criteria matched.`,
          'apply'
        );
      }
    }, 1200);
